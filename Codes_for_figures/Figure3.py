...
plt.scatter(x=df['tau'], y=df['deltaH_decomposition(meV/atom)'], c='black', alpha=0.5)
#axs[0] = plt.scatter(x=df['t'], y=df['deltaH_decomposition(meV/atom)'])
plt.xlabel(r"$\mathregular{\tau}$", **fontdict)
plt.xticks(**fontdict)
#plt.ylabel(r"$\mathregular {\Delta H_{decomp}-T\Delta S_{mix}}$"' (meV/atom)', **fontdict)
plt.ylabel(r"$\mathregular {\Delta H_{decomp}}$"' (meV/atom)', **fontdict)
...
selected.to_csv('lower right.csv', index=False)
plt.yticks(**fontdict)
plt.axvline(x=4.18, color='red', linestyle='--', linewidth=2)
plt.axhline(y=0, color='red', linestyle='--', linewidth=2)
x, y = df['tau'], df['deltaG (meV/atom)']
#plt.fill_between(x, y, where=(x >= 0) & (x <= 4.18) & (y >= -150) & (y <= 0), facecolor='gray', alpha=0.3)
plt.ylim(-110, 110)
